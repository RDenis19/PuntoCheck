...
class OrganizationService {
  ...
  /// Intenta extraer lon/lat desde GeoJSON o WKT.
  List<double>? _extractLonLat(dynamic geo) {
    ...
    if (geo is String && geo.contains('POINT')) {
      ...
      if (start != -1 && end != -1 && end > start + 1) {
        final parts = geo.substring(start + 1, end).split(RegExp(r'\\s+'));
        if (parts.length == 2) {
  ...
  Future<void> createBranch(Sucursales sucursal) async {
    try {
      ...
      // Convertimos a formato aceptado por PostGIS.
      final lonLat = _extractLonLat(sucursal.ubicacionCentral);
      if (lonLat != null) {
        final lon = lonLat[0];
        final lat = lonLat[1];
  ...
  Future<void> updateBranch(Sucursales branch) async {
    try {
      ...
      final lonLat = _extractLonLat(branch.ubicacionCentral);
      if (lonLat != null) {
        final lon = lonLat[0];
        final lat = lonLat[1];
      ...
      try {
        ...
        // Fallback: algunos setups aceptan mejor GeoJSON directo.
        final isGeoError = e.message.toLowerCase().contains('geography') ||
            e.message.toLowerCase().contains('geometry') ||
            e.message.toLowerCase().contains('ubicacion_central');

        if (lonLat != null && isGeoError) {
          final fallback = Map<String, dynamic>.from(updates);
          fallback['ubicacion_central'] = {
            'type': 'Point',
            'coordinates': [lonLat[0], lonLat[1]],
          };
          await supabase.from('sucursales').update(fallback).eq('id', branch.id);
          return;
        }
        rethrow;
      }
